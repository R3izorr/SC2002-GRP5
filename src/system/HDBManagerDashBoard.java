...
import system.service.common.ViewNotificationsService;
import system.service.common.ViewProfileService;
...
public class HDBManagerDashBoard extends SimpleMenu {
    ...
    public HDBManagerDashBoard(HDBManager manager,ProjectController projectController,ApplicationController applicationController,
        ...
        addOption(new UserOption("5", "Manage Applicant Applications", input -> new ManageApplicantApplicationsService(manager, applicationController, notificationController).run()));
        addOption(new UserOption("6", "Manage Withdrawn Applicant Requests", input -> new ManageApplicantWithdrawalsService(manager, applicationController, notificationController).run()));
        addOption(new UserOption("7", "Manage HDB Officer Registrations", input -> new ManageOfficerRegistrationsService(manager, userController.getAllOfficers(), projectController, notificationController).run()));
        ...
        addOption(new UserOption("11", "View Notifications", input -> new ViewNotificationsService( notificationController ,manager.getNric()).run()));
        addOption(new UserOption("12", "View Your Profile", input -> new ViewProfileService(manager).run()));
        addOption(new UserOption("13", "Change Password", input -> {
